    ...
    async def approve(self, ctx, suggestion_id: int, *, message=None):
        ...
        if not s_message:
            embed = discord.Embed(
                ...
                description="Is it Deleted? Can I see it? Yes? Maybe it doesn't exist.",
            )
            return await ctx.send(embed=embed)
        ...
        if fields == 4:
            embed.insert_field_at(
                index=2,
                name="Response",
                value=message if message else "No Response Provided.",
                inline=False,
            )
        else:
            embed.add_field(
    ...
    async def deny(self, ctx, suggestion_id: int, *, message=None):
        ...
        if not s_message:
            embed = discord.Embed(
                colour=self.bot.error_color,
                title=f"Message not found.",
                description="Is it Deleted? Maybe I can't see it? Or Perhaps it doesn't exist.",
            )
            return await ctx.send(embed=embed)
        embed = s_message.embeds[0]
        fields = len(embed.fields)
